...
private[caffe] class CaffeProcessor[T1, T2](val sources: Array[DataSource[T1, T2]],
  ...
  //initialize sources
  for (source <- sources) {
    val a: Boolean = source.init
    if (!source.init()) {
      throw new Exception("Failed to initialize data source")
    }
  }

  @volatile var validationResults: ArrayBuffer[Any] = new ArrayBuffer[Any]
  ...
  @volatile var results: ArrayList[Any] = new ArrayList[Any]
    ...
    try {
      ...
      for (it <- initIter until maxIter if (tpl != STOP_MARK)) {
        ...
        if (validationTime) {
          ...
          //validationResults += validationLine
